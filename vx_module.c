...
static PyObject *vx_get_linecoloffset_of_chs(PyObject *self, PyObject *args)
{
	PyObject *capsule;
	Window *window;
	PyObject *ret;
	...
	if (!PyArg_ParseTuple(args, "Os|i:get_linecoloffset_of_str", &capsule, &ch, &forwards))
		return NULL;
	WINDOW_FROM_CAPSULE;
	r = get_rowcoloffset_of_chars(window->buffer->text, ch, &l, &c, &o, forwards);
	if (r == -1)
		o = -1;
...
static PyObject *vx_get_chars_linecol_window(PyObject *self, PyObject *args)
{
	PyObject *capsule;
	Window *window;
	PyObject *ret;
...
static PyObject *vx_get_chars_linecol_to_linecol_window(PyObject *self, PyObject *args)
{
	PyObject *capsule;
	Window *window;
	PyObject *ret;
	...
	if (!PyArg_ParseTuple(args, "Oiiii:get_chars_linecol_to_linecol_window", &capsule, &rowa, &cola, &rowb, &colb))
		return NULL;
	WINDOW_FROM_CAPSULE;
	--rowa, --cola;
	--rowb, --colb;
	ch = get_chars_rowcol_to_rowcol(window->buffer->text, rowa, cola, rowb, colb);
	ret = Py_BuildValue("s", ch);
	return ret;
}
...
static PyMethodDef VxMethods[] = {
	...
	{"clear_window", vx_clear_window, METH_VARARGS,
	 "Clear a window"},
	{"get_linecoloffset_of_str", vx_get_linecoloffset_of_chs, METH_VARARGS,
	...
	{"get_str_linecol_window", vx_get_chars_linecol_window, METH_VARARGS,
	 "Get a string at a linecol with a length"},
	{"get_str_linecol_to_linecol_window", vx_get_chars_linecol_to_linecol_window, METH_VARARGS,
	 "Get a string at a linecol to a linecol"},
